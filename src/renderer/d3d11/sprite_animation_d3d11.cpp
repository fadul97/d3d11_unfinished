...
joj::D3D11SpriteAnimation::D3D11SpriteAnimation()
    : m_total_frames(0), m_columns(0), m_rows(0), m_delay(0.0f),
    m_current_frame(0), m_time_elapsed(0.0f), m_uv_offset(0.0f, 0.0f),
    m_cell_size(1.0f, 1.0f)
{
}

joj::D3D11SpriteAnimation::D3D11SpriteAnimation(i32 total_frames, i32 columns, i32 rows, u32 frames_per_second)
    : m_total_frames(total_frames), m_columns(columns), m_rows(rows), m_delay(1.0f / static_cast<f32>(frames_per_second))
{
    m_current_frame = 0;
    m_time_elapsed = 0.0f;
