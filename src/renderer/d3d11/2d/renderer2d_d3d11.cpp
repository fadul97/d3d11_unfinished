...
joj::D3D11Renderer2D::D3D11Renderer2D()
{
	m_context = nullptr;
	m_device = nullptr;
...
joj::ErrorCode joj::D3D11Renderer2D::init(WindowData& window)
{
	...
	swap_chain_desc.SampleDesc.Count = 1;                                                   // Samples per pixel (antialiasing)
	swap_chain_desc.SampleDesc.Quality = 0;                                                 // Level of image quality
...
void joj::D3D11Renderer2D::clear(f32 r, f32 g, f32 b, f32 a)
{
...
void joj::D3D11Renderer2D::render()
{
	JTODO();
}

void joj::D3D11Renderer2D::swap_buffers()
{
...
void joj::D3D11Renderer2D::set_primitive_topology(D3D_PRIMITIVE_TOPOLOGY topology) const
{
	m_device_context->IASetPrimitiveTopology(topology);
}

void joj::D3D11Renderer2D::set_rasterizer_fill_mode(RasterizerFillMode mode)
{
	switch (mode)
	{
