...
joj::D3D11Texture::D3D11Texture(const wchar_t* filepath)
    : m_srv(nullptr)
{
...
void joj::D3D11Texture::bind(u32 start_slot, u32 num_views)
{
    joj::Engine::s_renderer->get_device_context()->PSSetShaderResources(start_slot, num_views, &m_srv);
}

void joj::D3D11Texture::unbind()
{
    ID3D11ShaderResourceView* null_srv[1] = { nullptr };
    joj::Engine::s_renderer->get_device_context()->PSSetShaderResources(0, 1, null_srv);
}

joj::ErrorCode joj::D3D11Texture::create(const wchar_t* filepath)
{
    if (DirectX::CreateDDSTextureFromFile(
        joj::Engine::s_renderer->get_device(),
        filepath,
